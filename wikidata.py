...
def query_monuments(qid, lang):
    result = query_wikidata("SELECT DISTINCT ?item ?itemLabel ?local ?localLabel ?coord "
                            ...
                            "INCLUDE %items. "
                            "?item wdt:P625 ?coord. "
                            "{ ?item p:P131/ps:P131 ?local. ?local wdt:P31 wd:Q1649296. } "
                            "UNION "
                            "{ ?item p:P131/ps:P131 [wdt:P131 ?local]. ?local wdt:P31 wd:Q1649296. } "
                            ...
                            "'"+lang+",en,pt-br,pt,es,fr,de,ja,[AUTO_LANGUAGE]'.} "
...
def query_monument(qid, lang):
    result = query_wikidata("SELECT DISTINCT ?item ?itemLabel ?coord ?endereço ?localLabel ?estadoLabel ?paísLabel "
                            ...
                            "?P8517 ?P1766 ?P1801 ?P3451 ?P5775 ?P8592 ?P9721 ?P9906 "
                            "WITH { "
                            ...
                            "?P18 ?P3311 ?P4291 ?P8517 ?P1766 ?P1801 "
                            "?P3451 ?P5775 ?P8592 ?P9721 ?P9906 "
                            "WHERE { "
                            "BIND(wd:"+qid+" AS ?item) "
                            "?item wdt:P625 ?coord. "
                            "OPTIONAL{?item wdt:P6375 ?endereço} "
                            "{ ?item p:P131/ps:P131 ?local. ?local wdt:P31 wd:Q1649296. ?local wdt:P131 ?estado. "
                            "?estado wdt:P31 wd:Q485258.} "
                            "UNION "
                            "{ ?item p:P131/ps:P131 [wdt:P131 ?local]. "
                            "BIND(?local as ?estado) ?estado wdt:P31 wd:Q1649296. } "
                            "?item wdt:P17 ?país. "
                            "OPTIONAL {?item wdt:P18 ?P18} "
                            "OPTIONAL {?item wdt:P5775 ?P5775} "
                            "OPTIONAL {?item wdt:P9721 ?P9721} "
                            ...
                            "?tombamento_id ?tombamento ?número_de_inventário ?P18 ?P5775 ?P9721 ?P9906"
                            "?P1801 ?P1766 ?P8592 ?P3451 ?P4291 ?P8517 ?P3311 } AS %item "
                            "WHERE { "
                            "INCLUDE %item. "
                            "SERVICE wikibase:label { bd:serviceParam wikibase:language '"+
